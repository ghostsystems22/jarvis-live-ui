...
type ProductivityPage = 'overview' | 'tasks' | 'projects' | 'gantt' | 'cockpit';
...
export default function ProductivitySection({ model, error, today, onReload, onSetCapacity }: Props) {
  ...
  const projectTasks = selectedProject ? activeTasks.filter((task) => task.fields.project?.includes(selectedProject.id)) : activeTasks;
  ...
  async function completeTask(task: ApiRecord<TaskFields>) {
    setSaving(true);
    try {
      await cadenceApi.update('Tasks', task.id, { status: 'Terminé', completedAt: today });
  ...
  async function updateTask(task: ApiRecord<TaskFields>, field: keyof TaskFields, value: unknown) {
    setSaving(true);
    try {
      await cadenceApi.update('Tasks', task.id, { [field]: value });
      showToast('Saved ✓');
      await onReload();
    } catch {
